...
def printOutput(arg_signature, arg_format, arg_quiet):
    ...
    if arg_format == "base64":
        ...
        binary_data = base64.b64encode(arg_signature)
    elif arg_format == "hex":
        import binascii

        binary_data = binascii.hexlify(arg_signature)
    elif arg_format == "raw":
        sys.stdout.buffer.write(arg_signature)
    elif arg_format == "decimal":
        print(toInt(arg_signature))
    else:
        print("Unknown output format specified " + arg_format, file=sys.stderr)
...
if keySize is None:
    print("Please specify the key size and/or the modulus", file=sys.stderr)
    sys.exit(1)
...
if args.variant == 1:
    ...
    print("Unsupported signature method", file=sys.stderr)
    sys.exit(1)
...
if signature is None:
    print("Cannot generate the signature.", end="", file=sys.stderr)
    if (public_exponent <= 3) and (keySize < 4096):
        print(" (Key size is too small?)", end="", file=sys.stderr)
